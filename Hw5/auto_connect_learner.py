...
class ExMap(list):
	...
	Attributes = [ 'horizontal_change', 'lower_tri_ratio', 'upper_tri_ratio', 'diag_ratio']#, 'count_ratio']#, 'horizontal_change', 'vertical_change']
...
if __name__ == '__main__':
	...
	# for x in range(5):
	# 	for y in range(5):
	# 		pass#ExMap.Attributes.append( (x,y) )

	T_COUNT = 10
	Q_COUNT = 10
	T_SET_FILENAME = 'rt_training_set.txt'
	Q_SET_FILENAME = 'rt_query_set.txt'
	ROUNDS = 10
	...
	for r in range(ROUNDS):
		try:
			os.remove(T_SET_FILENAME)
		except:
			pass
		try:
			os.remove(Q_SET_FILENAME)
		except:
			pass


		make_training_set(Q_SET_FILENAME, Q_COUNT)
		make_training_set(T_SET_FILENAME,T_COUNT)
		attributes = ExMap.Attributes

		report = 'Time: %s\n' % datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
		report += "Training Set File: %s\nQuery Set File:%s\n\n" % (T_SET_FILENAME, Q_SET_FILENAME)
		report += "Testing %d examples using attributes: %s\n" % (T_COUNT, attributes)

		dtree = DecisionTreeLearning(read_examples(T_SET_FILENAME), attributes, [])
		with open ("example_tree.txt", "w") as f:
			f.write(str(dtree))

		successes, attempts = 0, 0
		for m in read_test_map(Q_SET_FILENAME):
			attempts += 1
			m.lame_check_connectedness()
			report += '==========================\n'
			report += 'Query #%d Map:\n' % attempts
			...
			path = None
			try:
				t = dtree.determine_connectedness(m)
				path = dtree.last_traverse
		...
		report += "Successes: %d\n" % successes
		report += "Failures: %d\n" % (attempts - successes)
		report += "Success rate: %f" % (float(successes)/attempts)

		r_success_rate += float(successes) / attempts

		with open('report%f.txt' % time.time(), 'w') as f:
			f.write(report)

	print "With %d examples and %d queries after %d rounds, average success rate is %f" % ( T_COUNT, Q_COUNT, ROUNDS, r_success_rate/ROUNDS)